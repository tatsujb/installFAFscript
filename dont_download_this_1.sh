...
then echo "[$(date --rfc-3339=seconds)] T2 editing debian sources : stretch already correct" >> $HOME/faf.sh-?.?.log;
else echo "[$(date --rfc-3339=seconds)] T2 editing debian sources : stretch edited" >> $HOME/faf.sh-?.?.log;
...
then echo "[$(date --rfc-3339=seconds)] T2 editing debian sources : stretch/updates already correct" >> $HOME/faf.sh-?.?.log;
else echo "[$(date --rfc-3339=seconds)] T2 editing debian sources : stretch/updates edited" >> $HOME/faf.sh-?.?.log
...
then echo "[$(date --rfc-3339=seconds)] T2 editing debian sources : proposed already present" >> $HOME/faf.sh-?.?.log;
else donwload_country=$(grep "deb http://ftp." /etc/apt/sources.list | head -1 | cut -d. -f2);
echo "[$(date --rfc-3339=seconds)] T1 editing debian sources : added proposed" >> $HOME/faf.sh-?.?.log;
...
then echo "[$(date --rfc-3339=seconds)] T2 enabled partners" >> $HOME/faf.sh-?.?.log;
...
echo "[$(date --rfc-3339=seconds)] T2 finished succesfully" >> $HOME/faf.sh-?.?.log;
