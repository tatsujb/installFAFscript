...
echo "_______________________________________________________________________________________________________" >> ~/'fafstack-'$faf_sh_version'.log'
echo "-------------------------------------------------------------------------------------------------------" >> ~/'fafstack-'$faf_sh_version'.log'
unameOut="$(uname -s)"
case "${unameOut}" in
	Linux*)
echo "[$(date --rfc-3339=seconds)] T1 New log file. fafSTACK version "$faf_sh_version" running : "$unameOut  >> ~/'fafstack-'$faf_sh_version'.log';;
	Darwin*)
...
echo "[$(date --rfc-3339=seconds)] T1 New log file. fafSTACK version "$faf_sh_version". FAILIURE. MAC UNSUPPORTED. "$unameOut  >> ~/'fafstack-'$faf_sh_version'.log'
exit 1;;
	CYGWIN*)
...
echo "[$(date --rfc-3339=seconds)] T1 New log file. fafSTACK version "$faf_sh_version". FAILIURE. WINDOWS UNSUPPORTED. "$unameOut  >> ~/'fafstack-'$faf_sh_version'.log'
exit 1;;
	MINGW*)
echo "Hello, you can on MinGW you cannot run Forged Alliance, this script is of no use to you."
echo "[$(date --rfc-3339=seconds)] T1 New log file. fafSTACK version "$faf_sh_version". FAILIURE. MINGW UNSUPPORTED. "$unameOut  >> ~/'fafstack-'$faf_sh_version'.log'
exit 1
esac
...
if [ -f /etc/os-release ]; then
	...
	echo "Distribution name + version + kernel version + architecture : "$operating_system" "$os_version" "$(uname -rm) >> ~/'fafstack-'$faf_sh_version'.log'
	case "${operating_system}" in
		Ubuntu*)
echo "-------------------------------------------------------------------------------------------------------" >> ~/'fafstack-'$faf_sh_version'.log'
echo "Hard storage setup :" >> ~/'fafstack-'$faf_sh_version'.log'
echo "_______________________________________________________________________________________________________" >> ~/'fafstack-'$faf_sh_version'.log'
lsblk_ouput=$(lsblk | grep -v 'loop')
echo "$lsblk_ouput" >> ~/'fafstack-'$faf_sh_version'.log'
dfh_ouput=$(df -h --total | grep -v 'loop')
echo "$dfh_ouput" >> ~/'fafstack-'$faf_sh_version'.log'
echo "_______________________________________________________________________________________________________" >> ~/'fafstack-'$faf_sh_version'.log'
echo "" >> ~/'fafstack-'$faf_sh_version'.log'
...
echo "[$(date --rfc-3339=seconds)] T1 Steam credentials entrusted to scritp" >> ~/'fafstack-'$faf_sh_version'.log' # NOTE THAT THIS IS NOT MY IDEAL SOLUTION BUT I HAVENT YET FOUND BETTER
sudo echo "[$(date --rfc-3339=seconds)] sudo priveledges entrusted to script" >> ~/'fafstack-'$faf_sh_version'.log' # AGAIN, SUGGESTIONS WELCOME...
...
then
	...
	echo "[$(date --rfc-3339=seconds)] T1 Gallium Nine chosen" >> ~/'fafstack-'$faf_sh_version'.log'
else
	...
	gallium_nine=false
	echo "[$(date --rfc-3339=seconds)] T1 Vanilla Proton chosen" >> ~/'fafstack-'$faf_sh_version'.log'
fi
if grep -Fxq "# deb http://archive.canonical.com/ubuntu cosmic partner" /etc/apt/sources.list
then
	echo "[$(date --rfc-3339=seconds)] T1 enabled partners" >> ~/'fafstack-'$faf_sh_version'.log'
	...
	echo "[$(date --rfc-3339=seconds)] T1 did not enable partners, hoping it was already enabled." >> ~/'fafstack-'$faf_sh_version'.log'
fi
...
then
	...
	echo "[$(date --rfc-3339=seconds)] T1 steam is already installed, proceeding..." >> ~/'fafstack-'$faf_sh_version'.log'
else
	echo "[$(date --rfc-3339=seconds)] T1 steam was not yet installed, installing..." >> ~/'fafstack-'$faf_sh_version'.log'
...
if [ $(command -v curl) ]
then
	echo "[$(date --rfc-3339=seconds)] T1 curl is already installed, proceeding..." >> ~/'fafstack-'$faf_sh_version'.log'
	echo "curl is already installed, proceeding..."
else
	echo "[$(date --rfc-3339=seconds)] T1 curl was not yet installed, installing..." >> ~/'fafstack-'$faf_sh_version'.log'
	to_be_installed=$((to_be_installed+2))
fi
if [ $(command -v pv) ]
then
	echo "[$(date --rfc-3339=seconds)] T1 pv is already installed, proceeding..." >> ~/'fafstack-'$faf_sh_version'.log'
	echo "pv is already installed, proceeding..."
else
	echo "[$(date --rfc-3339=seconds)] T1 pv was not yet installed, installing..." >> ~/'fafstack-'$faf_sh_version'.log'
...
echo "";
echo "";
echo "[$(date --rfc-3339=seconds)] T2 running steam" >> ~/fafstack-'$faf_sh_version'.log;
if '$gallium_nine' ;
...
echo "starting Steam...";
echo "[$(date --rfc-3339=seconds)] T2 starting Steam..." >> ~/fafstack-'$faf_sh_version'.log;
steam -login '$steam_user_name' '$steam_password';
echo "starting Forged Alliance Download...";
echo "[$(date --rfc-3339=seconds)] T2 starting Forged Alliance Download..." >> ~/fafstack-'$faf_sh_version'.log;
...
echo "[$(date --rfc-3339=seconds)] T2 steamCMD terminated (in a good way), starting FA to finalize install" >> ~/fafstack-'$faf_sh_version'.log;
...
echo "[$(date --rfc-3339=seconds)] T2 making map & mods symbolic links" >> ~/fafstack-'$faf_sh_version'.log;
if [ -d ~/.steam/steam/steamapps ] ;
then steamapps="steamapps";
echo "[$(date --rfc-3339=seconds)] T2 steamapps found" >> ~/fafstack-'$faf_sh_version'.log;
if [ -d ~/.steam/steam/steamapps/common/Supreme* ] ;
then echo "[$(date --rfc-3339=seconds)] T2 Supreme Commander folder found" >> ~/fafstack-'$faf_sh_version'.log;
...
else echo "[$(date --rfc-3339=seconds)] T2 Supreme Commander folder not found" >> ~/fafstack-'$faf_sh_version'.log;
fi;
if [ -d ~/.steam/steam/steamapps/compatdata/9420/pfx/drive_c/users/steamuser ] ;
then echo "[$(date --rfc-3339=seconds)] T2 steamuser profile folder found" >> ~/fafstack-'$faf_sh_version'.log;
cd ~/.steam/steam/steamapps/compatdata/9420/pfx/drive_c/users/steamuser;
rm -rf My\ Documents;
mkdir My\ Documents;
cd My\ Documents;
ln -s ~/My\ Games/ My\ Games;
else echo "[$(date --rfc-3339=seconds)] T2 steamuser profile folder not found" >> ~/fafstack-'$faf_sh_version'.log;
fi;
...
else echo "[$(date --rfc-3339=seconds)] T2 neither cammel case (SteamApps), nor lowercase (steamapps) found" >> ~/fafstack-'$faf_sh_version'.log;
...
[[ ( -d ~/.steam/steam/steamapps/common/Proton ) || ( -d ~/.steam/steam/SteamApps/common/Proton ) ]] && echo "[$(date --rfc-3339=seconds)] T2 PROTON folder found" >> ~/fafstack-'$faf_sh_version'.log || echo "[$(date --rfc-3339=seconds)] T2 Proton folder not found" >> ~/fafstack-'$faf_sh_version'.log;
source ~/.bashrc;
eval "$(cat ~/.bashrc | tail -n +10)";
echo "Finished thread two (install & run steam, steamcmd, FA) without issue, starting (FAF)...";
echo "[$(date --rfc-3339=seconds)] T2 Finished thread two (install & run steam, steamcmd, FA) without issue, starting (FAF)..." >> ~/fafstack-'$faf_sh_version'.log;
...
echo "[$(date --rfc-3339=seconds)] T1 start of second thread did not crash first thread" >> ~/'fafstack-'$faf_sh_version'.log'
if ! dpkg-query -W -f='${Status}' lib32gcc1 | grep "ok installed"
then
	echo "[$(date --rfc-3339=seconds)] T1 lib32gcc1 was not yet installed, installing..." >> ~/'fafstack-'$faf_sh_version'.log'
	sudo apt install -y lib32gcc1
else
	echo "[$(date --rfc-3339=seconds)] T1 lib32gcc1 is already installed, proceeding..." >> ~/'fafstack-'$faf_sh_version'.log'
	echo "lib32gcc1 is already installed, proceeding..."
fi
if [ $(command -v steamcmd) ]
then
	echo "[$(date --rfc-3339=seconds)] T1 steam CMD is already installed, proceeding..." >> ~/'fafstack-'$faf_sh_version'.log'
	echo "steam CMD is already installed, proceeding..."
else
	echo "[$(date --rfc-3339=seconds)] T1 steam CMD was not yet installed, installing..." >> ~/'fafstack-'$faf_sh_version'.log'
...
then
	echo "[$(date --rfc-3339=seconds)] T1 libnss3 was not yet installed, installing..." >> ~/'fafstack-'$faf_sh_version'.log'
	sudo apt install -y libnss3-tools
else
	echo "[$(date --rfc-3339=seconds)] T1 libnss3 is already installed, proceeding..." >> ~/'fafstack-'$faf_sh_version'.log'
...
echo "[$(date --rfc-3339=seconds)] T1 Now probing the Java status of this OS..." >> ~/'fafstack-'$faf_sh_version'.log'
...
then
	...
	echo "[$(date --rfc-3339=seconds)] T1 Java already installed!" >> ~/'fafstack-'$faf_sh_version'.log'
    ...
    echo "[$(date --rfc-3339=seconds)] T1 specifically Java 10." >> ~/'fafstack-'$faf_sh_version'.log'
    if grep -q "export INSTALL4J_JAVA_HOME=/usr/lib/jvm/jdk-10.0.2" ~/.bashrc
    then
    	echo "Huzzah! Java looks all set, let's move on..."
    	echo "[$(date --rfc-3339=seconds)] T1 satisfied with Java config." >> ~/'fafstack-'$faf_sh_version'.log'
    	else
        	echo "Hmmm... we're not looking to hot on .bashrc, let's see if we can fix that..."
    	echo "[$(date --rfc-3339=seconds)] T1 but not correct config." >> ~/'fafstack-'$faf_sh_version'.log'
   	 ...
   	 echo "[$(date --rfc-3339=seconds)] T1 corrected Java config." >> ~/'fafstack-'$faf_sh_version'.log'
    	else
   	 ...
   	 echo "[$(date --rfc-3339=seconds)] T1 Abandoned on user demand, Java was ill-configured" >> ~/'fafstack-'$faf_sh_version'.log'
   	 exit 1
    	fi      	 
    	fi
	else
    ...
    then
    	echo "your Path is defined but Java is not in /usr/lib/jvm "
        	echo "[$(date --rfc-3339=seconds)] T1 potential unclean uninstall of Java, java detected set yet /usr/lib/jvm does not even exist" >> ~/'fafstack-'$faf_sh_version'.log'
    fi
    ...
    echo "[$(date --rfc-3339=seconds)] T1 Incorrect Java version, exiting!" >> ~/'fafstack-'$faf_sh_version'.log'
    	exit 1
	fi
else
	# Download & install java 10 open jdk
	echo "Java 10 installation procedure..."
	echo "[$(date --rfc-3339=seconds)] T1 Java 10 installing..." >> ~/'fafstack-'$faf_sh_version'.log'
...
echo "[$(date --rfc-3339=seconds)] T1 installing DOWNLORD" >> ~/'fafstack-'$faf_sh_version'.log'
cd
mkdir faf
cd faf
rm -rf *
...
echo "[$(date --rfc-3339=seconds)] T1 Finished thread one. (proton/downlord/open-jdk/bashrc)" >> ~/'fafstack-'$faf_sh_version'.log'
;;
		"Linux Mint"*)
echo "-------------------------------------------------------------------------------------------------------" >> ~/'fafstack-'$faf_sh_version'.log'
echo "Hard storage setup :" >> ~/'fafstack-'$faf_sh_version'.log'
...
;;
		"elementary OS"*)
		echo "Hi, youre on elementary OS, best of luck"
echo "-------------------------------------------------------------------------------------------------------" >> ~/'fafstack-'$faf_sh_version'.log'
echo "Hard storage setup :" >> ~/'fafstack-'$faf_sh_version'.log'
echo "_______________________________________________________________________________________________________" >> ~/'fafstack-'$faf_sh_version'.log'
lsblk_ouput=$(lsblk | grep -v 'loop')
...
#                                                                                                                   	#
#########################################################################################################################
xterm -T "install & run steam, steamcmd, FA" -e '[ '$gallium_nine' ] && echo "installing gallium/proton and running steam...";
echo "expecting you to type in Forged Alliances Launch options";
echo "reminder : look in your home folder, theres a file there with the contents to be pasted";
echo "once thats done edit steam settings in order to enable Proton for all games";
[ '$gallium_nine' ] && echo "it should have Gallium pre-selected already, this is what you want, just tick the box next to it.";
...
echo "Finished thread two (install & run steam, steamcmd, FA) without issue, starting (FAF)...";
echo "[$(date --rfc-3339=seconds)] T2 Finished thread two (install & run steam, steamcmd, FA) without issue, starting (FAF)..." >> ~/fafstack-'$faf_sh_version'.log;
xterm -T "(FAF)" -e "cd ~/faf; ./downlords-faf-client";' &
# source ~/.bashrc be it in main thread or in tab, has no effect the user will have to run it himself.
# end of second thread
# finalising apt-install
echo "[$(date --rfc-3339=seconds)] T1 start of second thread did not crash first thread" >> ~/'fafstack-'$faf_sh_version'.log'
...
exit 0;;
		Kubuntu*)
		echo "Hi, youre on Kubuntu, best of luck"
echo "-------------------------------------------------------------------------------------------------------" >> ~/'fafstack-'$faf_sh_version'.log'
echo "Hard storage setup :" >> ~/'fafstack-'$faf_sh_version'.log'
echo "_______________________________________________________________________________________________________" >> ~/'fafstack-'$faf_sh_version'.log'
lsblk_ouput=$(lsblk | grep -v 'loop')
...
#                                                                                                                   	#
#########################################################################################################################
konsole -e /bin/bash --rcfile <(echo '[ '$gallium_nine' ] && echo "installing gallium/proton and running steam...";
echo "expecting you to type in Forged Alliances Launch options";
echo "reminder : look in your home folder, theres a file there with the contents to be pasted";
echo "once thats done edit steam settings in order to enable Proton for all games";
[ '$gallium_nine' ] && echo "it should have Gallium pre-selected already, this is what you want, just tick the box next to it.";
...
echo "Finished thread two (install & run steam, steamcmd, FA) without issue, starting (FAF)...";
echo "[$(date --rfc-3339=seconds)] T2 Finished thread two (install & run steam, steamcmd, FA) without issue, starting (FAF)..." >> ~/fafstack-'$faf_sh_version'.log;
konsole -e /bin/bash --rcfile <(echo "cd ~/faf; ./downlords-faf-client; exit 0") &') &
# source ~/.bashrc be it in main thread or in tab, has no effect the user will have to run it himself.
# end of second thread
# finalising apt-install
echo "[$(date --rfc-3339=seconds)] T1 start of second thread did not crash first thread" >> ~/'fafstack-'$faf_sh_version'.log'
...
exit 0;;
		Lubuntu*)
		echo "Hi, youre on Linux Mint, best of luck"
echo "-------------------------------------------------------------------------------------------------------" >> ~/'fafstack-'$faf_sh_version'.log'
echo "Hard storage setup :" >> ~/'fafstack-'$faf_sh_version'.log'
echo "_______________________________________________________________________________________________________" >> ~/'fafstack-'$faf_sh_version'.log'
lsblk_ouput=$(lsblk | grep -v 'loop')
...
echo "steam password :"
read -s steam_password
echo "[$(date --rfc-3339=seconds)] T1 Steam credentials entrusted to scritp" >> ~/'faf.sh-'$faf_sh_version'.log' # NOTE THAT THIS IS NOT MY IDEAL SOLUTION BUT I HAVENT YET FOUND BETTER
sudo echo "[$(date --rfc-3339=seconds)] sudo priveledges entrusted to script" >> ~/'faf.sh-'$faf_sh_version'.log' # AGAIN, SUGGESTIONS WELCOME...
if (whiptail --title "Use Gallium Nine Proton instead of vanilla Proton?" --yesno "If you don't know what Gallium Nine is or don't care, choose \"No\"." 10 100)
then
	gallium_nine=true
	echo 'PROTON_NO_ESYNC=1, PROTON_DUMP_DEBUG_COMMANDS=1, PROTON_USE_GALLIUM_NINE=1, PROTON_GALLIUM_NINE_MODULEPATH="/usr/lib/i386-linux-gnu/d3d/d3dadapter9.so.1:/usr/lib/x86_64-linux-gnu/d3d/d3dadapter9.so.1" %command%' > ~/"the contents of this file are to be pasted in the forged alliance properties launch options"
	echo "[$(date --rfc-3339=seconds)] T1 Gallium Nine chosen" >> ~/'faf.sh-'$faf_sh_version'.log'
else
	echo 'PROTON_NO_ESYNC=1, PROTON_DUMP_DEBUG_COMMANDS=1 %command%' > ~/"the contents of this file are to be pasted in the forged alliance properties launch options"
	gallium_nine=false
	echo "[$(date --rfc-3339=seconds)] T1 Vanilla Proton chosen" >> ~/'faf.sh-'$faf_sh_version'.log'
fi
if grep -Fxq "# deb http://archive.canonical.com/ubuntu cosmic partner" /etc/apt/sources.list
then
...
echo "";
echo "";
echo "[$(date --rfc-3339=seconds)] T2 running steam" >> ~/faf.sh-'$faf_sh_version'.log;
if '$gallium_nine' ;
then mkdir -p ~/.steam/compatibilitytools.d;
proton_version=$(curl -v --silent https://api.github.com/repos/popsUlfr/Proton/releases 2>&1 | grep "tag_name" | head -n 1 | cut -f4,4 -d"\"");
proton_temp_value=$(curl -v --silent https://api.github.com/repos/popsUlfr/Proton/releases 2>&1 | grep "target_commitish" | head -n 1 | cut -f4,4 -d"\"" | sed "s/[^_]\+/\L\u&/g");
...
echo "starting Steam...";
echo "[$(date --rfc-3339=seconds)] T2 starting Steam..." >> ~/faf.sh-'$faf_sh_version'.log;
steam -login '$steam_user_name' '$steam_password';
echo "starting Forged Alliance Download...";
echo "[$(date --rfc-3339=seconds)] T2 starting Forged Alliance Download..." >> ~/faf.sh-'$faf_sh_version'.log;
steamcmd +login '$steam_user_name' '$steam_password' +@sSteamCmdForcePlatformType windows +app_update 9420 validate +quit;
echo "steamCMD terminated (in a good way), starting FA to finalize install";
...
if [ -d ~/.steam/steam/steamapps ] ;
then steamapps="steamapps";
echo "[$(date --rfc-3339=seconds)] T2 steamapps found" >> ~/faf.sh-'$faf_sh_version'.log;
if [ -d ~/.steam/steam/steamapps/common/Supreme* ] ;
then echo "[$(date --rfc-3339=seconds)] T2 Supreme Commander folder found" >> ~/faf.sh-'$faf_sh_version'.log;
cd ~/.steam/steam/steamapps/common/Supreme\ Commander\ Forged\ Alliance;
ln -s ~/My\ Games/Gas\ Powered\ Games/Supreme\ Commander\ Forged\ Alliance/Maps/ Maps;
ln -s ~/My\ Games/Gas\ Powered\ Games/Supreme\ Commander\ Forged\ Alliance/Mods/ Mods;
else echo "[$(date --rfc-3339=seconds)] T2 Supreme Commander folder not found" >> ~/faf.sh-'$faf_sh_version'.log;
fi;
if [ -d ~/.steam/steam/steamapps/compatdata/9420/pfx/drive_c/users/steamuser ] ;
then echo "[$(date --rfc-3339=seconds)] T2 steamuser profile folder found" >> ~/faf.sh-'$faf_sh_version'.log;
cd ~/.steam/steam/steamapps/compatdata/9420/pfx/drive_c/users/steamuser;
rm -rf My\ Documents;
mkdir My\ Documents;
cd My\ Documents;
ln -s ~/My\ Games/ My\ Games;
else echo "[$(date --rfc-3339=seconds)] T2 steamuser profile folder not found" >> ~/faf.sh-'$faf_sh_version'.log;
fi;
elif [ -d ~/.steam/steam/SteamApps ];
then steamapps="SteamApps";
echo "[$(date --rfc-3339=seconds)] T2 SteamApps found" >> ~/faf.sh-'$faf_sh_version'.log;
...
else echo "[$(date --rfc-3339=seconds)] T2 neither cammel case (SteamApps), nor lowercase (steamapps) found" >> ~/faf.sh-'$faf_sh_version'.log;
echo "UNACCEPTABLEEEEEEEEEEEEEEEE";
exit 1;
fi;
if '$gallium_nine' ;
...
eval "$(cat ~/.bashrc | tail -n +10)";
echo "Finished thread two (install & run steam, steamcmd, FA) without issue, starting (FAF)...";
echo "[$(date --rfc-3339=seconds)] T2 Finished thread two (install & run steam, steamcmd, FA) without issue, starting (FAF)..." >> ~/faf.sh-'$faf_sh_version'.log;
gnome-terminal --tab --title="(FAF)" -- bash -c "cd ~/faf; ./downlords-faf-client";'
# source ~/.bashrc be it in main thread or in tab, has no effect the user will have to run it himself.
# end of second thread
# finalising apt-install
...
# Java fix-me block
echo "Now probing the Java status of this OS..."
echo "[$(date --rfc-3339=seconds)] T1 Now probing the Java status of this OS..." >> ~/'faf.sh-'$faf_sh_version'.log'
if [[ $(command -v java) ]] || [[ $(type -p java) ]] || [[ -n "$JAVA_HOME" ]] && [[ -x "$JAVA_HOME/bin/java" ]]
then
	echo "UH-OH, Java is already installed!"
	echo "This is suboptimal, crossing fingers for correct version Java version (E.G. version number 10)..."
    ...
    echo "Huzzah! you have the correct version of Java, here's hoping for the best!"
    echo "Checking, if .bashrc config is also correct..."
    echo "[$(date --rfc-3339=seconds)] T1 specifically Java 10." >> ~/'faf.sh-'$faf_sh_version'.log'
    if grep -q "export INSTALL4J_JAVA_HOME=/usr/lib/jvm/jdk-10.0.2" ~/.bashrc
    then
    	echo "Huzzah! Java looks all set, let's move on..."
    	echo "[$(date --rfc-3339=seconds)] T1 satisfied with Java config." >> ~/'faf.sh-'$faf_sh_version'.log'
    	else
        	echo "Hmmm... we're not looking to hot on .bashrc, let's see if we can fix that..."
    	echo "[$(date --rfc-3339=seconds)] T1 but not correct config." >> ~/'faf.sh-'$faf_sh_version'.log'
        	if (whiptail --title "Entered : \"Java 10 present but .bashrc not correctly configured use-case\"" --yesno "Attempt .bashrc automated correct? \n\"No\" will close this script \n\"Yes\" will automatically edit bashrc \n(keep in mind this script was written by a donkey...)" 10 100)
    	then
   	 echo "OK! \".bashrc\" edited!"
   	 echo "assuming Java is all set, let's move on..."
   	 echo "[$(date --rfc-3339=seconds)] T1 corrected Java config." >> ~/'faf.sh-'$faf_sh_version'.log'
    	else
   	 echo "You're probably right to choose this..."
   	 echo "please edit .bashrc yourself or remove Java 10 and start over."
   	 echo "exiting upon demand..."
   	 echo "[$(date --rfc-3339=seconds)] T1 Abandoned on user demand, Java was ill-configured" >> ~/'faf.sh-'$faf_sh_version'.log'
   	 exit 1
    	fi      	 
    	fi
	else
    ...
    then
    	echo "your Path is defined but Java is not in /usr/lib/jvm "
        	echo "[$(date --rfc-3339=seconds)] T1 potential unclean uninstall of Java, java detected set yet /usr/lib/jvm does not even exist" >> ~/'faf.sh-'$faf_sh_version'.log'
    fi
    	echo "You have a version of Java that is not Java 10"
    	echo "This is problematic as this case is not yet handled by this script (though it could easily be!)"
    	echo "Feel free to contribute at : https://github.com/tatsujb/installFAFscript"
    	...
    	echo "then, re-run this script"
    	echo "FAF-stack not installed, exiting..."
    echo "[$(date --rfc-3339=seconds)] T1 Incorrect Java version, exiting!" >> ~/'faf.sh-'$faf_sh_version'.log'
    	exit 1
	fi
else
	# Download & install java 10 open jdk
...
exit 0	
    ...
    echo "You're probably right to choose this..."
    echo "exiting upon demand..."
    echo "[$(date --rfc-3339=seconds)] T1 Abandoned on user demand, OS version too experimental" >> ~/'fafstack-'$faf_sh_version'.log'
    exit 1
	else
    echo "OK! fingers crossed!"
    echo "continuing..."
    echo "[$(date --rfc-3339=seconds)] T1 wrong distribution, user-chosen continue." >> ~/'fafstack-'$faf_sh_version'.log'
	fi
