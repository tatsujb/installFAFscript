...
case "${unameOut}" in
	Linux*)
echo "[$(date --rfc-3339=seconds)] T1 New log file. faf.sh version "$faf_sh_version" running : "$unameOut  >> ~/'faf.sh-'$faf_sh_version'.log';;
	Darwin*)
...
echo "[$(date --rfc-3339=seconds)] T1 New log file. faf.sh version "$faf_sh_version". FAILIURE. MAC UNSUPPORTED. "$unameOut  >> ~/'faf.sh-'$faf_sh_version'.log'
exit 1;;
	CYGWIN*)
...
echo "[$(date --rfc-3339=seconds)] T1 New log file. faf.sh version "$faf_sh_version". FAILIURE. WINDOWS UNSUPPORTED. "$unameOut  >> ~/'faf.sh-'$faf_sh_version'.log'
exit 1;;
	MINGW*)
echo "Hello, you can on MinGW you cannot run Forged Alliance, this script is of no use to you."
echo "[$(date --rfc-3339=seconds)] T1 New log file. faf.sh version "$faf_sh_version". FAILIURE. MINGW UNSUPPORTED. "$unameOut  >> ~/'faf.sh-'$faf_sh_version'.log'
exit 1
esac
...
exit 0	
    ...
    echo "You're probably right to choose this..."
    echo "exiting upon demand..."
    echo "[$(date --rfc-3339=seconds)] T1 Abandoned on user demand, OS version too experimental" >> ~/'faf.sh-'$faf_sh_version'.log'
    exit 1
	else
    echo "OK! fingers crossed!"
    echo "continuing..."
    echo "[$(date --rfc-3339=seconds)] T1 wrong distribution, user-chosen continue." >> ~/'faf.sh-'$faf_sh_version'.log'
	fi
