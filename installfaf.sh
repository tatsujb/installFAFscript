...
case "${unameOut}" in
	Linux*)
echo "[$(date --rfc-3339=seconds)] New log file. fafSTACK version "$faf_stack_version" running : "$unameOut  >> ~/'fafstack-'$faf_stack_version'.log';;
	Darwin*)
...
echo "[$(date --rfc-3339=seconds)] New log file. fafSTACK version "$faf_stack_version". FAILIURE. MAC UNSUPPORTED. "$unameOut  >> ~/'fafstack-'$faf_stack_version'.log'
exit 1;;
	CYGWIN*)
...
echo "[$(date --rfc-3339=seconds)] New log file. fafSTACK version "$faf_stack_version". FAILIURE. WINDOWS UNSUPPORTED. "$unameOut  >> ~/'fafstack-'$faf_stack_version'.log'
exit 1;;
	MINGW*)
echo "Hello, you can on MinGW you cannot run Forged Alliance, this script is of no use to you."
echo "[$(date --rfc-3339=seconds)] New log file. fafSTACK version "$faf_stack_version". FAILIURE. MINGW UNSUPPORTED. "$unameOut  >> ~/'fafstack-'$faf_stack_version'.log'
exit 1
esac
...
if [ -f /etc/os-release ]; then
    ...
    echo "[$(date --rfc-3339=seconds)] Abandoned on user demand, OS version too experimental" >> ~/'fafstack-'$faf_stack_version'.log'
    exit 1
	else
    echo "OK! fingers crossed!"
    echo "continuing..."
    echo "[$(date --rfc-3339=seconds)] wrong distribution, user-chosen continue." >> ~/'fafstack-'$faf_stack_version'.log'
	fi
...
echo "[$(date --rfc-3339=seconds)] Steam credentials entrusted to scritp" >> ~/'fafstack-'$faf_stack_version'.log' # NOTE THAT THIS IS NOT MY IDEAL SOLUTION BUT I HAVENT YET FOUND BETTER
...
then
	...
	echo "[$(date --rfc-3339=seconds)] Gallium Nine chosen" >> ~/'fafstack-'$faf_stack_version'.log'
else
	...
	gallium_nine=false
	echo "[$(date --rfc-3339=seconds)] Vanilla Proton chosen" >> ~/'fafstack-'$faf_stack_version'.log'
fi
if grep -Fxq "# deb http://archive.canonical.com/ubuntu cosmic partner" /etc/apt/sources.list
then
	echo "enabled partners" >> ~/'fafstack-'$faf_stack_version'.log'
	...
	echo "[$(date --rfc-3339=seconds)] did not enable partners, hoping it was already enabled." >> ~/'fafstack-'$faf_stack_version'.log'
fi
...
then
	...
	echo "[$(date --rfc-3339=seconds)] steam is already installed, proceeding..." >> ~/'fafstack-'$faf_stack_version'.log'
else
	echo "[$(date --rfc-3339=seconds)] steam was not yet installed, installing..." >> ~/'fafstack-'$faf_stack_version'.log'
...
if [ $(command -v curl) ]
then
	echo "[$(date --rfc-3339=seconds)] curl is already installed, proceeding..." >> ~/'fafstack-'$faf_stack_version'.log'
	echo "curl is already installed, proceeding..."
else
	echo "[$(date --rfc-3339=seconds)] curl was not yet installed, installing..." >> ~/'fafstack-'$faf_stack_version'.log'
	to_be_installed=$((to_be_installed+2))
fi
if [ $(command -v pv) ]
then
	echo "[$(date --rfc-3339=seconds)] pv is already installed, proceeding..." >> ~/'fafstack-'$faf_stack_version'.log'
	echo "pv is already installed, proceeding..."
else
	echo "[$(date --rfc-3339=seconds)] pv was not yet installed, installing..." >> ~/'fafstack-'$faf_stack_version'.log'
...
echo "";
echo "";
echo "[$(date --rfc-3339=seconds)] running steam" >> ~/fafstack-'$faf_stack_version'.log;
if '$gallium_nine' ;
...
echo "starting Steam...";
echo "[$(date --rfc-3339=seconds)] starting Steam..." >> ~/fafstack-'$faf_stack_version'.log;
steam -login '$steam_user_name' '$steam_password';
echo "starting Forged Alliance Download...";
echo "[$(date --rfc-3339=seconds)] starting Forged Alliance Download..." >> ~/fafstack-'$faf_stack_version'.log;
...
echo "[$(date --rfc-3339=seconds)] steamCMD terminated (in a good way), starting FA to finalize install" >> ~/fafstack-'$faf_stack_version'.log;
...
else echo "[$(date --rfc-3339=seconds)] Supreme Commander folder not found" >> ~/fafstack-'$faf_stack_version'.log;
fi;
if [ -d ~/.steam/steam/steamapps/compatdata/9420/pfx/drive_c/users/steamuser ] ;
then echo "[$(date --rfc-3339=seconds)] steamuser profile folder found" >> ~/fafstack-'$faf_stack_version'.log;
...
then
	echo "[$(date --rfc-3339=seconds)] steam CMD is already installed, proceeding..." >> ~/'fafstack-'$faf_stack_version'.log'
	echo "steam CMD is already installed, proceeding..."
else
	echo "[$(date --rfc-3339=seconds)] steam CMD was not yet installed, installing..." >> ~/'fafstack-'$faf_stack_version'.log'
...
then
	echo "[$(date --rfc-3339=seconds)] libnss3 was not yet installed, installing..." >> ~/'fafstack-'$faf_stack_version'.log'
	sudo apt install -y libnss3-tools
else
	echo "[$(date --rfc-3339=seconds)] libnss3 is already installed, proceeding..." >> ~/'fafstack-'$faf_stack_version'.log'
...
echo "[$(date --rfc-3339=seconds)] Now probing the Java status of this OS..." >> ~/'fafstack-'$faf_stack_version'.log'
...
then
	...
	echo "[$(date --rfc-3339=seconds)] Java already installed!" >> ~/'fafstack-'$faf_stack_version'.log'
    ...
    echo "[$(date --rfc-3339=seconds)] specifically Java 10." >> ~/'fafstack-'$faf_stack_version'.log'
    if grep -q "export INSTALL4J_JAVA_HOME=/usr/lib/jvm/jdk-10.0.2" ~/.bashrc
    then
    	echo "Huzzah! Java looks all set, let's move on..."
    	echo "[$(date --rfc-3339=seconds)] satisfied with Java config." >> ~/'fafstack-'$faf_stack_version'.log'
    	else
        	echo "Hmmm... we're not looking to hot on .bashrc, let's see if we can fix that..."
    	echo "[$(date --rfc-3339=seconds)] but not correct config." >> ~/'fafstack-'$faf_stack_version'.log'
   	 ...
   	 echo "[$(date --rfc-3339=seconds)] corrected Java config." >> ~/'fafstack-'$faf_stack_version'.log'
    	else
   	 echo "You're probably right to choose this..."
   	 echo "please edit .bashrc yourself or remove Java 10 and start over."
   	 echo "exiting upon demand..."
   	 echo "[$(date --rfc-3339=seconds)] Abandoned on user demand, Java was ill-configured" >> ~/'fafstack-'$faf_stack_version'.log'
   	 exit 1
    	fi      	 
    	fi
	else
    ...
    then
    	echo "your Path is defined but Java is not in /usr/lib/jvm "
        	echo "[$(date --rfc-3339=seconds)] potential unclean uninstall of Java, java detected set yet /usr/lib/jvm does not even exist" >> ~/'fafstack-'$faf_stack_version'.log'
    fi
    ...
    echo "[$(date --rfc-3339=seconds)] Incorrect Java version, exiting!" >> ~/'fafstack-'$faf_stack_version'.log'
    	exit 1
	fi
else
	# Download & install java 10 open jdk
	echo "Java 10 installation procedure..."
	echo "[$(date --rfc-3339=seconds)] Java 10 installing..." >> ~/'fafstack-'$faf_stack_version'.log'
...
echo "[$(date --rfc-3339=seconds)] installing DOWNLORD" >> ~/'fafstack-'$faf_stack_version'.log'
cd
mkdir faf
cd faf
rm -rf *
